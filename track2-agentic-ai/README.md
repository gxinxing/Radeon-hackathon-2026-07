...
> and Dify integration — all powered by Qwen2.5-7B fine-tuned on AMD Radeon GPU.
...
- **Dify Integration** — Single HTTP call (`/api/agent/run`) runs the full pipeline;
...
| Dify integration | FastAPI + OpenAPI spec | `/api/agent/run` + `/api/agent/reward` endpoints |
