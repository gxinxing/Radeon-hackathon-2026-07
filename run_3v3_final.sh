...
# Team A: 3 ONNX robots (A_attacker has ball authority)
/opt/venv/bin/python /workspace/radeon-repo/match_worker.py \
    --role A_attacker --has-ball --port $PORT \
    --onnx $ONNX --init-pos -3 0 0.7 > /tmp/match_final_a1.log 2>&1 &
/opt/venv/bin/python /workspace/radeon-repo/match_worker.py \
    --role A_defender --port $PORT \
    --onnx $ONNX --init-pos -4 -1.5 0.7 > /tmp/match_final_a2.log 2>&1 &
/opt/venv/bin/python /workspace/radeon-repo/match_worker.py \
    --role A_keeper --port $PORT \
    --onnx $ONNX --init-pos -6 0 0.7 > /tmp/match_final_a3.log 2>&1 &

# Team B: 3 rule-based robots (NO --has-ball)
/opt/venv/bin/python /workspace/radeon-repo/match_worker.py \
    --role B_attacker --port $PORT \
