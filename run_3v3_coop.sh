...
PORT=9883
ONNX=/workspace/radeon-repo/models/chase_v9_policy.onnx
...
# Team A: 3 robots with ONNX model (A_attacker has ball authority)
...
# Team B: 3 rule-based robots (NO --has-ball to avoid ball state conflict)
/opt/venv/bin/python /workspace/radeon-repo/match_worker.py \
    --role B_attacker --port $PORT --init-pos 3 0 0.7 > /tmp/onnx_b1.log 2>&1 &
/opt/venv/bin/python /workspace/radeon-repo/match_worker.py \
